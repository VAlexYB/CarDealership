...
const getConfigurations = async (configuration, step = 1) => {
    ...
    if (step === 5 ) {
        ...
        if (entities.length === 0) {
            ...
            entities.forEach(entity => {
                ...
                equipmentCard.querySelector('.config-part-title').textContent = entity.equipment.name + " " + entity.equipment.releaseYear;
                article.addEventListener('click', function() {
    ...
    if (step === 6 ) {
        ...
        if (entities.length === 0) {
            ...
            entities.forEach(entity => {
                ...
                engineCard.querySelector('.config-part-title').textContent = entity.engine.engineType + ", " + entity.engine.power + " л.с, " + (entity.engine.transmissionType === "Механическая" ? "МТ" : "АТ");
                article.addEventListener('click', function() {
    ...
    if (step === 8 ) {
        ...
        entities.forEach(entity => {
            ...
            + entity.bodyType + ", " + entity.color + ", "  + entity.equipment.name + ", " + entity.equipment.releaseYear + ", " + entity.price + "₽";
